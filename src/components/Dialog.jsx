...
export default function DetailDialog(props) {
  ...
  function videoForward() {
    if (details.videos && videoIndex < details.videos.length - 1)
  ...
  function imgForward() {
    if (details.photos && imgIndex < details.photos.length - 1)
                  ...
                  <>
                    ...
                    {details.score && (
                      <div className={classes.scoreWrapper}>
                        <Typography>
                          Score:
                          {details.score.length == 0 ? " No score found" : " "}
                        </Typography>
                        {details.score.map((obj, index) => (
                          <div key={index} className={classes.score}>
                            <img
                              src={
                                obj.provider_type.includes("imdb")
                                  ? "https://img.icons8.com/color/48/000000/imdb.png"
                                  : "https://img.icons8.com/color/48/000000/tomato.png"
                              }
                      ...
                      <div className={classes.contentBackground}>
                        <Paper variant="outlined" square>
                          ...
                          >
                            <Tab
                              label="Trailers"
                              disabled={details.videos ? false : true}
                            />
                            <Tab
                              label="Photos"
                              disabled={details.photos ? false : true}
                            />
                          </Tabs>
                        </Paper>
                        <TabPanel
                              ...
                              style={
                                details.videos && {
                                  transform: `translateX(-${
                                    (videoIndex / details.videos.length) * 100
                                  }%)`,
                                }
                              }
                            >
                          ...
                          {details.videos && (
                            <>
                              ...
                              <IconButton
                                ...
                                color="primary"
                                disabled={
                                  videoIndex >= details.videos.length - 1
                                }
                          ...
                          <div className={classes.outerWrapper}>
                            <div
                              ...
                              style={
                                details.photos && {
                                  transform: `translateX(-${
                                    (imgIndex / details.photos.length) * 100
                                  }%)`,
                                }
                              }
                            >
                          ...
                          {details.photos && (
                            <>
                              ...
                              <IconButton
                                className={classes.arrowForward}
                                color="primary"
                                disabled={imgIndex >= details.photos.length - 1}
                                onClick={imgForward}
                              >
                                <ArrowForwardIosIcon />
                              </IconButton>
                    ...
                    <div className={classes.contentBackground}>
                      ...
                      >
                        {details.offers ? (
                          Object.keys(details.offers).map((key, index) => (
                            <Typography key={index}>
                              {details.offers[key].url ? (
                                <Link
                                  href={details.offers[key].url}
                                  color="primary"
                                >
                                  {key}:
                                </Link>
                              ...
                              {details.offers[key].rent
                                ? ` From USD ${details.offers[key].rent}`
                                : ` Not available`}
                            </Typography>
                          ))
                        ) : (
                        ...
                        {details.offers ? (
                          Object.keys(details.offers).map((key, index) => (
                            <Typography key={index}>
                              ...
                              {details.offers[key].buy
                                ? ` From USD ${details.offers[key].buy}`
                                : ` Not available`}
                            </Typography>
                          ))
                        ) : (
                    ...
                    {!isLogIn && (
                      <Typography color="textSecondary" variant="caption">
                        Want to find out other online video services available
                        for this movie? Be a member now!
                      </Typography>
                    )}
                  </>
                ) : (
            ...
            <div className={classes.content}>
              {comments ? (
                comments.length > 0 ? (
                  ...
                  <div>
                    No comments yet, be the first people to comment this movie!
                  </div>
                )
              ) : (
                "Loading Database"
